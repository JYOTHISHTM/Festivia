...
class AuthRepository {
  ...
  // async findByEmail(email: string, type: "user" | "creator") {
  //   return type === "user"
  //     ? await UserModel.findOne({ email: new RegExp(`^${email}$`, 'i') })
  //     : await CreatorModel.findOne({ email: new RegExp(`^${email}$`, 'i') });
  // }
  ...
  async saveOTP(email: string, otp: string, expiresAt: Date) {
    console.log(`💾 Saving OTP for ${email}:`, { otp }); // ✅ Debug log
  ...
  ////////////////
  // async sendOtp(email: string, type: "user" | "creator"): Promise<boolean> {
  //   try {
  //     if (type === "user") {

  //       const user = await this.findByEmail(email, type);
  //     } else {
  //       const creator = await this.findByEmail(email, type);
  ...
  // AuthRepository
  async findByGoogleId(googleId: string): Promise<ICreator | null> {
