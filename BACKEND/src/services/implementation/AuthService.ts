...
class AuthService {
  ...
  async login(email: string, password: string, role: "user" | "creator") {
    ...
    return {
      token,
      refreshToken,
      user: { id: user._id, email: user.email, name: user.name }, // 🔥 Ensure `user` is correctly returned
    };
  }
  ...
  async sendOtp(email: string,type:"user" | "creator"): Promise<{ message: string; otp: string }> {
    ...
    return { message: "OTP sent to email", otp }; // ✅ return OTP here
  }
  
  async verifyOtp(email: string, otp: string, type: "user" | "creator"): Promise<boolean> {
    const record = await AuthRepository.findByEmail(email, type); // or "creator"
    
    if (!record || record.otp !== otp) {
      return false;
    }
  
    // await AuthRepository.deleteByEmail(email, type); // ❌ Don't delete OTP now
    return true;
  }
