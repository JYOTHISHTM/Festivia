...
const VerifyOtp: React.FC = () => {
  ...
  const handleResendOtp = async () => {
    ...
    inputRefs.current[0]?.focus(); // Set focus back to first input
    startTimer();
    try {
        ...
        <form onSubmit={handleSubmit} className="flex flex-col items-center">
          ...
          <button
            ...
            className={`w-full py-2 rounded ${
              otpExpired ? "bg-gray-400 cursor-not-allowed" : "bg-green-500 hover:bg-green-600 text-white"
            }`}
            disabled={otpExpired}
          >
            Verify OTP
        ...
        <div className="text-center mt-4">
          ...
          <button
            className={`mt-2 px-4 py-2 rounded ${
              isResendDisabled ? "bg-gray-400 cursor-not-allowed" : "bg-blue-500 hover:bg-blue-600 text-white"
            }`}
