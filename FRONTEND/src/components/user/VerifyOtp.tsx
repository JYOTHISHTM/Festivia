...
const VerifyOtp: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post("http://localhost:5001/users/verify-otp", { email, otp: otpCode });
  ...
  const handleResendOtp = async () => {
    try {
      await axios.post("http://localhost:5001/users/resend-otp", { email });
      startTimer();
    } catch (err: any) {
    ...
    <div className="flex justify-center items-center min-h-screen p-6">
      <div className="w-96 p-6 bg-white shadow-lg rounded-lg">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col items-center">
          ...
          <button type="submit" className="w-full bg-green-500 text-white py-2 rounded" disabled={otpExpired}>
            Verify OTP
          </button>
        </form>
